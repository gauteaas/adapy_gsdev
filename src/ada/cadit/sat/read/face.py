...
class PlateFactory:
    ...
    def get_face_name_and_points(self, face_data_str: str) -> tuple[str, list[tuple[float]]]:
        ...
        r = res[self.name_idx]
        name = self.sat_store.get_name(r)
        if not name.startswith("FACE"):
            logger.info(f"Dace_refs not starting with 'FACE'. Found {name}. It's not from Sesam")
...
class CurvedPlateFactory:
    ...
    def get_face_name(self, face_data_str: str) -> str:
        res = face_data_str.strip().split()
        r = res[self.name_idx]
        name = self.sat_store.get_name(r)
        if not name.startswith("FACE"):
    ...
    def get_face_edges(self, face_data_str: str) -> list[list[str]]:
        res = face_data_str.strip().split()
        edges = self.get_edges(res)
        return edges

    # def get_surface_data(self, face_data_str: str) -> list[list[str]]:
    #     res = face_data_str.strip().split()
    #     edges = self.get_edges(res)
    #     return edges

    def get_edge_curves(self, coedges):
        ...
        for coedge in coedges:
            edges.append(self.sat_store.get(coedge[edge_idx]))

        for i, edge in enumerate(edges):
            curve_list = self.sat_store.get(edge[curve_idx], split_lines=True)
            curves.append("\n".join(curve_list))

            v1 = self.sat_store.get(edge[start_idx])
            v2 = self.sat_store.get(edge[stop_idx])
            p1 = [float(x) for x in self.sat_store.get(v1[point_idx])[6:9]]
            p2 = [float(x) for x in self.sat_store.get(v2[point_idx])[6:9]]

            if 'forward' in coedges[i][coedge_sense_idx]:
                # points.append((p1, p2))
                orient.append(True)
            else:
                # points.append((p2, p1))
        ...
        return edges,curves, points, orient
